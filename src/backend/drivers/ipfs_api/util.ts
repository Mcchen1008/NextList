...
export class ClientIpfs {
  ...
  private async post<T = any>(endpoint: string, args: string[]): Promise<T> {
    ...
    const resp = await fetch(
      `${this.endpoint}/api/v0/${endpoint}?${qs.toString()}`,
      {
        method: "POST",
      },
    )
    if (resp.status >= 400) {
      throw new Error(
        `[IPFS] ${resp.status} ${await resp.text().catch(() => "")}`,
      )
    }
  ...
  async add(content: Buffer, fileName: string): Promise<string> {
    ...
    const resp = await fetch(`${this.endpoint}/api/v0/add`, {
      method: "POST",
      body: form,
    })
    const data: IpfsAddResp = await resp
      .json()
      .catch(() => ({ Hash: "" }) as any)
