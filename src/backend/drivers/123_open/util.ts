...
export class Client123Open {
  ...
  private async getAccessToken(): Promise<void> {
    ...
    if (
      ...
      const resp = await fetch(
        `${this.addition.api_url_address}?${qs.toString()}`,
      )
      ...
      const err =
        data.error_description || data.text || data.message || data.error
    ...
    if (this.addition.client_id && this.addition.client_secret) {
      ...
      const data: Resp123 & { data?: AccessTokenData123 } = await resp
        .json()
        .catch(() => ({ code: -1 }) as any)
      if (data.code !== 0) throw new Error(`[123Open] ${data.message}`)
      if (!data.data?.access_token)
        throw new Error("[123Open] empty access token")
  ...
  async getFiles(parentFileId: string): Promise<File123[]> {
    ...
    do {
      ...
      const resp = await this.request<FileListData123>(
        `${API}/api/v2/file/list?${qs.toString()}`,
        "GET",
      )
  ...
  async getDownloadUrl(fileId: number): Promise<string> {
    ...
    const resp = await this.request<DownloadInfoData123>(
      `${API}/api/v1/file/download_info?${qs.toString()}`,
      "GET",
    )
    if (resp.code !== 0) throw new Error(`[123Open] ${resp.message}`)
  ...
  async mkdir(parentId: string, name: string): Promise<void> {
    const resp = await this.request(`${API}/upload/v1/file/mkdir`, "POST", {
      parentID: parentId,
      name,
    })
    if (resp.code !== 0) throw new Error(`[123Open] ${resp.message}`)
  ...
  async rename(fileId: number, fileName: string): Promise<void> {
    const resp = await this.request(`${API}/api/v1/file/name`, "PUT", {
      fileId,
      fileName,
    })
    if (resp.code !== 0) throw new Error(`[123Open] ${resp.message}`)
  ...
  async remove(fileId: number): Promise<void> {
    const resp = await this.request(`${API}/api/v1/file/trash`, "POST", {
      fileIDs: [fileId],
    })
    if (resp.code !== 0) throw new Error(`[123Open] ${resp.message}`)
  }
