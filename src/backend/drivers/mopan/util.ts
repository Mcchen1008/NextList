...
export class MoPanClient {
  ...
  private async request<T = any>(
    ...
    if (!res.ok) {
      throw new Error(`[MoPan] Request failed: ${res.status} ${res.statusText}`)
    }
  ...
  async queryFiles(
    ...
    return this.request<QueryFilesResp>(MoPanProxyFamily + "/queryFiles", data)
  }
  ...
  async commitMultiUploadFile(uploadFileId: string): Promise<CommitUploadResp> {
    const data = {
      uploadFileId,
    }
