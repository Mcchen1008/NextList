#!/usr/bin/env node
/**
 * NextList 一键部署脚本（Cloudflare Workers）
 *
 * wrangler.toml 只声明绑定（[[kv_namespaces]] binding = "NEXTLIST_KV"），
 * **不存储 id** —— wrangler 4.x 的 Automatic provisioning 会在部署时自动
 * 创建/关联同名 KV namespace，无需手动填写 id。
 *
 * 本脚本额外做两件事：
 *   1. 检测云端是否已有 NEXTLIST_KV namespace；没有则显式创建
...
const KV_TITLE = "NEXTLIST_KV"

const args = process.argv.slice(2)
if (args.includes("--help") || args.includes("-h")) {
  console.log(`
NextList 一键部署脚本（KV 自动绑定，无需手动填写 id）
...
/** 确保 NEXTLIST_KV namespace 存在（不存在则创建）。
...
function main() {
  ...
  console.log("\n✅ 部署完成！")
  console.log("   验证：访问 https://<你的域名>/api/health 应返回 NextList")
}
