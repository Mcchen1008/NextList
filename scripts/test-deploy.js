...
const mockList = `
🌀 Listing namespaces with title filter "OpenListTeam-Nextlist"
┌──────────────────────────────────────┬──────────────────────────────┐
│ id                                   │ title                        │
├──────────────────────────────────────┼──────────────────────────────┤
│ 0e48234248a84d4dbdc5a70e886773ea    │ nextlistpro-NEXTLIST_KV │
└──────────────────────────────────────┴──────────────────────────────┘
`
...
const found = Object.keys(map).find((t) => t.includes("NEXTLIST_KV"))
console.log("匹配:", found, "→ id:", found ? map[found] : null)

// 2. 模拟 create 输出
const mockCreate = `
🌀 Creating namespace with title "NEXTLIST_KV"
✨ Success!
...
console.log(
  "toml 其他内容保留:",
  updated.includes('name = "nextlistpro"') &&
    updated.includes('binding = "NEXTLIST_KV"'),
)
