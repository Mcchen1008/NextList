...
- [x] MCP (Model Context Protocol) - \_完整 JSON-RPC 2.0 实现：initialize 握手（协议版本协商 2025-03-26/2024-11-05）、ping、tools/list + tools/call（list_files / get_file_info / search_files / get_system_info 真实执行）、resources/read（storage/metrics + storage/list）、prompts/get（summarize_directory）；双传输：Streamable HTTP（POST /api/mcp）+ 旧版 SSE（GET /api/mcp/sse + POST /api/mcp/messages，keepalive）；鉴权复用 getUserFromContext（JWT / 静态 API Token / 启用的 guest），initialize 与 ping 免鉴权
