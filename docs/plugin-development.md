...
> ### ✅ NextList 兼容性说明
>
> 本文档描述的插件系统已由 **NextList**（OpenListNext 的 Node.js 全栈分支）完整移植。
> 插件清单格式（`plugin.json`）、权限字典、`OpenListPlugin` SDK、后台 API（`/admin/plugin/*`、`/public/plugins`）与 ZIP 打包规范**与上游完全一致**：
> 为 OpenListNext / NextList 开发的插件包可直接在两者之间互相安装运行（相互兼容）。
> 管理后台入口：**管理面板 -> 插件管理 (Plugins)**；示例插件见仓库 `plugins_src/` 与 `public/plugins/`。
